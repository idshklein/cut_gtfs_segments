...
prod2 <- prod1 %>% 
  ...
  #        counter_seg_third = cumsum(counter_second + counter_third),
  #        has_third = sum(counter_second + counter_third)>0) 
  mutate(counter_seg_second = cumsum(counter_second),
         has_second = sum(counter_second) > 0) %>% 
  ungroup() %>% 
...
final$first_order_cuts <- tmp %>% 
  ungroup() %>% 
  select(first_order_seg_id = counter_seg_first,
         from_cut = origin,
         to_cut = destination,
         shape_id,
         seg_UID,
