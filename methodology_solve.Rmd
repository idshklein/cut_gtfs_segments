...
mapview(lns[285,]) + pts
qqq <- st_split(lns[195,],pts[191,])
qqq %>% st_collection_extract(type = "LINESTRING") %>% pull(line)
#   st_sf() %>% 
...
# 
# ggccpp <- eds %>% 
#   mutate(res1 = map(line,~st_split(.x,nds) %>% st_collection_extract(type = "LINESTRING")),
#          res1class = map(res1,~class(.x[[1]])),
#          res2 = map2(res1,res1class, function(x,y){
#            if(y == "numeric"){
#              st_sfc(x) %>% st_cast("MULTILINESTRING")
#            }else{
#              x %>% st_cast("MULTILINESTRING")
#            }
#          })) %>% 
#   st_set_geometry("res2")
# 
# eds[192,"line"] %>% st_split(nds) %>% st_collection_extract(type = "LINESTRING") %>% summarise()
# geom_collec <- map(eds$line,~st_split(.x,nds) %>% st_collection_extract(type = "LINESTRING"))
# st_geometry_type(geom_collec[[192]])
```
